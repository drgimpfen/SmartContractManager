...
| **Mini-Epic 4.1** | Template Decomposition & Modal Modularization | **Completed** | 100% (Extracted modals from contract_detail into components, consolidated dashboard contract modal) |
| **Mini-Epic 4.2** | Provider Multi-Contract Analytics & Cashflow | **Planned (Next)** | 0% (Past 12M actuals + Next 12M forecast stacked chart, KPI summary for providers) |
...
- [x] Modularize `contract_detail.html` modals into reusable partials:
...
- [x] Consolidate contract creation modals: Replace inline modal in `dashboard.html` with the shared component `_contract_modal.html`.
- [x] Maintain 100% selector, ID, and event compatibility for frontend scripts (`contract-term.js`, `combobox.js`, `tag-picker.js`).
- [x] Verify test suite and ensure clean template rendering without DOM or layout regressions.
